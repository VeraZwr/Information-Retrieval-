...
def retrieve(index_files: str, words: list, splits: list, N, l_avg, verbose=False):
    res = []
    if len(words) == 0:
        return res
    a = words[0]
...
def cal_score(ld, tf, N, df, l_avg):
    # print(ld, tf, N, df, l_avg)
    return (math.log(N/df))*(K+1)*tf/(K*((1-b)+b*(ld/l_avg))+tf)


def rank(postings, N ,l_avg):
    res = {}
    for posting in postings:
        for info in posting:
            id, ld, tf = [int(x) for x in info.strip("\n").split("~")]
            df = len(posting)
            score = cal_score(ld, tf, N, df, l_avg)
...
if __name__ == "__main__":
    ...
    if mode == "-a":
        ...
        res = retrieve(files, query, spliting_words, N, l_avg, verbose)
        print("[INFO]----Results------", res[:20])
    else:
        print("[ERROR] missing boolean operator '-a' '-o' '-r' ")
        help()
