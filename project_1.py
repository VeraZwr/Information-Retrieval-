...
def clean_source(documents, d_length):
    cleaned_documents = []
    ...
    return cleaned_documents, d_length
...
def build_inverted_index_in_memory(inverted_index, single_doc):
    ...
    for token in single_doc[1]:
        if inverted_index.get(token, None) is not None:
            inverted_index[token].add("~".join([str(single_doc[0]), str(len(single_doc[1])), str(counter[token])])) # (id,body)->(id, doc length, term count)
        else:
...
def persist_memory_data(inverted_index, f_name):
    ...
    for key in sorted(inverted_index.keys()):
        f.write(key + "=" + " ".join(sorted(inverted_index.get(key), key=lambda s: int(s.split("~")[0]))) + "\n")
    f.close()
...
if __name__ == "__main__":
    ...
    for file in files:
        ...
        cleaned_docs, d_length = clean_source(docs, d_length)
        print(d_length)
        counter = 0
    ...
    f = open("document info.txt", "w")
    f.write(' '.join(ending_words)+"\n")
    f.write("Total document length:"+str(doc_number)+"\n")
