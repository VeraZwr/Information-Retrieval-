...
def retrieve(index_files: str, words: list, splits: list, N, N_ai, df_ai, l_avg, verbose=False, rank_alg="bm25"):
    res = []
    if len(words) == 0:
        return res
    a = words[0]
...
def cal_score_BM25_ai(ld, tf, N_ai, l_avg):
    # print(ld, tf, N, df, l_avg)
    return (math.log(N_ai/df_ai))*(K+1)*tf/(K*((1-b)+b*(ld/l_avg))+tf)
    ...
    for posting in postings: #['url~337~1', 'url~389~2'...]
        for info in posting:
            ...
            if rank_alg == "-bm25":
                ...
                score = cal_score_BM25_ai(ld, tf, N_ai, l_avg)
...
if __name__ == "__main__":
    ...
    f_ai = open("document info ai.txt", "r")
    f_ai_df = open("AiFileDF.txt", "r")
    document_info = f.readline().strip("\n").split(":-)")
    f_ai.readline()
    N = int(f.readline().strip("\n").split(":")[1])
    N_ai = int(f_ai.readline().strip("\n").split(":")[1])
    df_ai = int(f_ai_df.readline().strip("\n").split(" = ")[1])
    ...
    if rank_alg == "-bm25":
        res = retrieve(files, query, spliting_words, N, N_ai, df_ai, l_avg, verbose, rank_alg)
